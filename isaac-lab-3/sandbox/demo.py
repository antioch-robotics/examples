"""
A naked Isaac Lab session that idles forever, so the GUI is yours.
...
scene, drop in assets, press Play, and inspect prims with the full Isaac Lab
engine (isaaclab, torch, and friends) importable from its script editor. The
outer bound is `antioch run`'s --timeout (default 900 s); --seconds adds an
inner one when you want the script to exit on its own.
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Idle a naked Isaac Lab session for GUI use")
